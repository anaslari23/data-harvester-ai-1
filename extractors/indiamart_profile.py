...
class IndiaMartProfileScraper:
    """Scrape IndiaMart company profile pages for owner/turnover."""

    async def enrich_from_profile(
        self, company_name: str, data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Enrich company data from IndiaMart profile."""
        result = dict(data)
    ...
    async def _find_profile_url(
        ...
        """Search DDG for IndiaMart profile URL."""
        query = f'site:indiamart.com "{company_name}" {city} contact person'
                ...
                if r.status_code == 200:
                    ...
                    for item in soup.select("div.result__body")[:5]:
                        ...
                        if not link:
                            continue
                        href = link.get("href", "")
                        if "indiamart.com" in href:
                            if "/proddetail/" not in href and "/search." not in href:
                                if href.count("/") <= 5:
                                    return href
        except Exception:
            pass
    ...
    def _parse_profile(
            ...
            for pattern in contact_patterns:
                ...
                if m:
                    ...
                    if 4 < len(name) < 50:
                        ...
                        result["owner_role"] = "Proprietor"
                        break

        if not result.get("contact_name"):
            for el in soup.select(
                ".contact-person, .cp-name, [class*='contact'] strong, "
                "[class*='owner'], .propr-name, .contact-name"
            ):
        ...
        if not result.get("employee_count"):
            emp_m = re.search(
                r"(?:No\.\s+of\s+Employees?|Employees?|Staff)\s*:?\s*"
                r"([\d,]+\s*[-–to]*\s*[\d,]*\+?)",
                text,
                re.I,
            )
        ...
        if not result.get("industry_type"):
            nature_m = re.search(
                r"Nature\s+of\s+Business\s*:?\s*([A-Za-z\s/&]+?)(?:\n|<)", text, re.I
            )
            if nature_m:
        ...
        if gst_m:
            ...
            if existing:
                result["additional_info"] = f"{existing} | GST: {gst_m.group(1)}"
            else:
                result["additional_info"] = f"GST: {gst_m.group(1)}"
...
async def enrich_with_indiamart(
    ...
    scraper = IndiaMartProfileScraper()
